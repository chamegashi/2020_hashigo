<template>
  <div>
    ...
    <div id="backColorBox" class="backColorBox">
      <div v-for="(sentence, index) in vib_words" :key="index" class="show-sentence">
        <div v-for="(word, word_index) in sentence" :key="word_index" class="showword">
          <p v-if="word_index % 2 === 0" class="vib0">{{word}}</p>
          <p v-else-if="word_index % 2 === 1" class="vib1">{{word}}</p>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    moveToButtom () {
      var e = document.getElementById('backColorBox')
...
@keyframes move0 {
    1% {transform: translate(0.5px, 0px) rotateZ(0deg)}
    50% {transform: translate(0px, 0px) rotateZ(0deg)}
    51% {transform: translate(1px, 0px) rotateZ(0deg)}
    100% {transform: translate(0px, 0px) rotateZ(0deg)}
}

@keyframes move1 {
    25% {transform: translate(0px, 0px) rotateZ(0deg)}
    26% {transform: translate(1px, 0px) rotateZ(0deg)}
    75% {transform: translate(0px, 0px) rotateZ(0deg)}
    76% {transform: translate(1px, 0px) rotateZ(0deg)}
    100% {transform: translate(0.5px, 0px) rotateZ(0deg)}
}
